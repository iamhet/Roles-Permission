            ...
            success: function(response) {
                ...
                $.each(response.permission, function(key, value) {
                    ...
                    if ($.inArray(value['id'], response.rolePermissions) !== -1) {
                        data.push(
                            '<input type="checkbox" class="form-control-select" name="permission[]" value=' +
                            value['name'] + ' checked /> ' + value['name'] + '<br>');
                    } else {
                        data.push(
                            '<input type="checkbox" class="form-control-select" name="permission[]" value=' +
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $.each(response.permission, function(key, value) {
                    ...
                    if ($.inArray(value['id'], response.rolePermissions) !== -1 || $.inArray(value['id'],response.userPermission) !== -1) {
                        data.push(
                            '<input type="checkbox" class="form-control-select" name="permission[]" value=' +
                            value['name'] + ' checked /> ' + value['name'] + '<br>');
                    } else {
